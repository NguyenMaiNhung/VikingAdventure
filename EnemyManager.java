		...
		for (Crabby c : currentLevel.getCrabs())
			if (c.isActive())
				if (c.getState() != DEAD && c.getState() != HIT)
					if (playing.getPlayer().getPowerValue() >= 50 && attackBox.intersects(c.getHitbox())) {
						c.hurt(20);
                                                playing.getPlayer().changePower(-50);
						return;
					}
		...
		for (Pinkstar p : currentLevel.getPinkstars())
			if (p.isActive()) {
				if (p.getState() == ATTACK && p.getAniIndex() >= 3)
					...
					if (playing.getPlayer().getPowerValue() >= 50 && p.getState() != DEAD && p.getState() != HIT)
		...
		for (Shark s : currentLevel.getSharks())
			if (s.isActive()) {
				if (s.getState() != DEAD && s.getState() != HIT)
					if (playing.getPlayer().getPowerValue() >= 50 && attackBox.intersects(s.getHitbox())) {
						s.hurt(20);
                                                playing.getPlayer().changePower(-50);
						return;
					}
